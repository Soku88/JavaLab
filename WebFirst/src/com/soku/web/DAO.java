...
public class DAO {
	...
	public static int insHobby(HobbyEntity param) {
		Connection conn = null;
		PreparedStatement ps = null;
		String sql = "INSERT INTO t_hobby (id, hobby_nm) "
					+ "SELECT IFNULL(MAX(id), 0) + 1, ? FROM t_hobby ";
	...
	public static HobbyEntity selHobby(HobbyEntity param) {
		...
		String sql = "SELECT name FROM t_hobby WHERE id = ?";
	...
	public static List<HobbyEntity> selHobbyList() {
		...
		String sql = "SELECT id, hobby_nm FROM t_hobby";
